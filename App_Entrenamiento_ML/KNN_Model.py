...
class KNNWindow(QMainWindow, Ui_KNN):
    def __init__(self, dataset):
        super(KNNWindow, self).__init__()
        self.setupUi(self)
    ...
    #Definimos el modelo de K vecinos más cercanos
    def build_model(self):

        n_neighbors = int(self.kVecinosLineEdit.text())
        weight = self.WeightComboBox.currentText()
        algorithm = self.algoritmoComboBox.currentText()
        p_distancia = self.pComboBox.currentText()

        if p_distancia == "euclidean_distance (p=2)":
            p = 2
        else:
            p = 1

 
        self.model_knn = KNeighborsClassifier(n_neighbors=n_neighbors, weights=weight, algorithm=algorithm, p=p, metric='minkowski')
    ...
    def entrenar_modelo(self):
        ...
        if self.model_knn:
            self.model_knn.fit(self.x_train_norm, self.y_train) #(x, y)
            #visualizar el modelo KNN
        else:
    ...
    def mostrar_resultados(self):
        ...
        y_pred = self.model_knn.predict(self.x_test_norm)
        ...
        scoresTest = self.model_knn.score(self.x_test_norm, self.y_test)
    ...
    def mostrar_graficas(self):
        ...
        y_pred_proba_knn = self.model_knn.predict_proba(self.x_test_norm)
        ...
        for i, class_name in enumerate(class_names):
            fpr, tpr, thresholds = roc_curve(y_test_onehot.iloc[:, i], y_pred_proba_knn[:, i])
            roc_auc = auc(fpr, tpr)
        ...
        result = permutation_importance(self.model_knn, self.x_test_norm, self.y_test, n_repeats=10, random_state=42, n_jobs=2)

        knn_importances = pd.Series(result.importances_mean, index=self.feature_names)

        fig, ax = plt.subplots()
        knn_importances.plot.bar(yerr=result.importances_std, ax=ax)
        ax.set_title("Importancia de las características utilizando permutación en el modelo KNN")
    ...
    def descargar_modelo(self): #Guardar el modelo entrenado
        if self.model_knn:
            ...
            if file_path:
                joblib.dump(self.model_knn, file_path)
